...
export default function VendorsPageClient({
  ...
  initialVendors: (Vendor & { products: Product[] })[]
}) {
  // Transform to DemoVendor with optional display fields
  const seedVendors: DemoVendor[] = initialVendors.map((v) => ({
    ...v,
    orderCount: 0,
    lastOrdered: "Recently",
  }))

  // Fallback to demo data if no vendors
  const fallbackSeed: DemoVendor[] = [
  {
  ...
  const finalSeed = seedVendors.length > 0 ? seedVendors : fallbackSeed
...
// ---------------------------------------------------------------------------
  ...
  const [vendors, setVendors] = useState<DemoVendor[]>(finalSeed)
